...
Handle<Value> LibxlSheet::ReadNumber(const Arguments& args) {
  HandleScope scope;
  ...
  // TODO update when Format API implemented
  double data = obj->sheet->readNum(row, col, 0);
  char *result = (char *) malloc(sizeof(char) * 1024);
  sprintf(result, "%f", data);

  // __strdup is done because documentation says that data can be lost when
  // book->release() is done
  return scope.Close(stov8s(__strdup(result)));
}
...
void LibxlSheet::Initialize(Handle<Object> target) {
  ...
  t->PrototypeTemplate()->Set(
     String::NewSymbol("readNumber"),
     FunctionTemplate::New(ReadNumber)->GetFunction()
   );

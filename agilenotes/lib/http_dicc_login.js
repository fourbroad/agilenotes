...
var headers = { 'Host' : hostname, 'Content-Type' : 'application/json',
	'User-Agent' : 'Mozilla/5.0 (X11; Linux i686; rv:12.0) Gecko/20100101 Firefox/12.0' };

var options = { hostname : hostname, headers : headers, port : 80, path : '', method : 'GET' };
...
var saveUserInfo = function(provider, cookie_id, username, password, callback) {
	...
	var req = http.request(options, function(res) {
		...
		res.on('data', function(chunk) {
			...
			if (chunk.code == 1508) {
				if (cookie_id) {
					provider.update({ _id : new BSON.ObjectID(cookie_id) }, { "$set" : {
						content : JSON.stringify(res.headers['set-cookie']), login_time : new Date() } }, null,
						function(err, data) {
							if (err) {
								console.log(err);
							} else {
...
var coreLogout = function(provider, cookie_id, callback) {
	provider.remove({ type : '51dbc4e01097ed07eb000112', _id : BSON.ObjectID(cookie_id) }, null,
...
var coreLogin = function(provider, cookie_id, callback) {
	provider.findOne({ type : '51dbc4e01097ed07eb000112', _id : BSON.ObjectID(cookie_id) }, null, null, function(error,
			result) {
		if (result) {
...
function loginCore(callback) {
	...
	var req = http.request(options, function(res) {
		...
		res.on("end", function() {
			callback(false, res.headers['set-cookie']);
		});
	});
